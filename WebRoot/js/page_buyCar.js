	$(function(){
			...
			function getASum(obj){
				obj.next().children().text(($(obj).prev().find("span").text()*obj.find(".text_box").val()).toFixed(2));
			}
			 ...
			 function find(){
				...
				$.post("BuyCarServlet",data,function(rel){
					...
					for(var i=0;i<rel.length;i++){
						 str+="<div><input id='check1' type='checkbox' name='choose' value=''/><a href='#'>SuperClass旗舰店</a><a href='#'><img src='img/3.jpg'></a>  </div>";
						str+="<div class='div2'>";
						str+="<ul>";
						str+="<li id='ll1'><input class='good1'  type='checkbox' name='choose2' value=''/></li>";
						str+="<li><img style='width: 103px;height: 103px;' src='img/"+rel[i].picPath+"' /></li>";
						...
						str+="</ul>";
						str+="</li>";
						str+="<li class='cc2 c2'  >￥<span id='price'>"+rel[i].sellingPrice+"</span></li>";
						...
						str+="<li class='c2'><span class='price'>￥"+rel[i].sellingPrice*rel[i].productCount+"</span></li>";
						str+="<li class='c2'>";
						...
						sumPrice+=rel[i].sellingPrice*rel[i].productCount;
					}
				...
				stt+="已选<span class='ccc1' id='countSum'>"+sumCount+"</span>件商品  总计（不含运费）：￥<span class='ccc1' id='priceSum'>"+sumPrice+"</span> <span class='cc1'>去结算</span>";
				stt+="</div>";
			...
			var classs= document.getElementsByClassName('good1');
			...
			check1.onclick=function(){
					for(var i=0; i<classs.length;i++){
						classs[i].checked=check1.checked;
					}
			};
